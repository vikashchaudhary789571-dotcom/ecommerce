...
export function InPdfEditor(props) {
    ...
    useEffect(() => {
        const loadPdf = async (pwd = null) => {
            ...
            try {
                ...
                for (let i = 1; i <= loadedPdf.numPages; i++) {
                    ...
                    const items = textContent.items.map((item, idx) => {
                        ...
                        return {
                            ...
                            // ═══════════════════════════════════════════════════════════
                            // CRITICAL: Store complete font metrics for preservation
                            // ═══════════════════════════════════════════════════════════
                            fontMetrics: {
                                name: item.fontName,
                                size: fontSize,
                                ...
                                width: item.width,
                                height: item.height,
                                // Store transform matrix for exact positioning
                                scaleX: item.transform[0],
                                scaleY: item.transform[3],
                                skewX: item.transform[1],
                                skewY: item.transform[2]
                            },
                            hasChanged: false
                        };
                    });
                ...
                // ═══════════════════════════════════════════════════════════════════
                // CRITICAL: Log PDF metadata and font information for debugging
                // ═══════════════════════════════════════════════════════════════════
                ...
                // Extract unique fonts used across all pages
                const uniqueFonts = new Set();
                allPagesData.forEach(page => {
                    page.items.forEach(item => {
                        if (item.fontName) uniqueFonts.add(item.fontName);
                    });
                });
                console.log(`[PDF Analysis] Unique fonts detected: ${Array.from(uniqueFonts).join(', ')}`);
                
                // Log PDF version if available
                try {
                    const pdfInfo = await loadedPdf.getMetadata();
                    if (pdfInfo && pdfInfo.info) {
                        console.log(`[PDF Analysis] PDF Version: ${pdfInfo.info.PDFFormatVersion || 'Unknown'}`);
                        console.log(`[PDF Analysis] Producer: ${pdfInfo.info.Producer || 'Unknown'}`);
                        console.log(`[PDF Analysis] Creator: ${pdfInfo.info.Creator || 'Unknown'}`);
                    }
                } catch (metaErr) {
                    console.warn(`[PDF Analysis] Could not extract metadata:`, metaErr.message);
                }
    ...
    const handleDownload = async () => {
        ...
        try {
            ...
            const downloadUrl = `https://ecommerce-2sdf.onrender.com/api/statements/download-file?fileUrl=${encodeURIComponent(fileUrl)}`;
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (data.success) {
                console.log('[Save] ✓ PDF saved successfully');
                console.log('[Save] 📊 Stats:', data.stats);
                if (data.metadata) {
                    console.log('[Save] 📋 Metadata preserved:', data.metadata);
                }
    ...
    const handleTransformWithPrecision = async (editedTxns, _originalTxns, tableClosingBalance) => {
        ...
        try {
            ...
            if (transformResponse.metadata) {
                console.log(`[Transform] 📋 Original PDF metadata preserved:`, transformResponse.metadata);
            }
